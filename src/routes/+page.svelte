<script lang="ts">
	...
	const translations: Translations = {
		fr: {
			...
			registerSuccess:
				'Compte créé avec succès! Veuillez télécharger et conserver votre fichier de clé en lieu sûr.',
		...
		en: {
			...
			registerSuccess:
				'Account created successfully! Please download and keep your key file in a safe place.',
			registerError: 'Error: ',
	...
	async function onLoginSubmit(event: Event) {
		...
		if (!result.success) {
			...
			if (zodErrors.fieldErrors.file) errors.file = zodErrors.fieldErrors.file.join(', ');
			return;
		}
...
<div
	class="flex items-center justify-center min-h-screen px-4 py-8 sm:px-6 md:py-12"
...
>
	...
	<div
		class="w-full max-w-xs sm:max-w-sm md:max-w-md rounded-[0.5rem] shadow-lg overflow-hidden mx-auto"
		style="background-color: #ced7e1;"
	>
		<!-- Card Header -->
		<div class="p-3 sm:p-4 md:p-6 border-b" style="border-color: #474b4f;">
			<h2
				class="text-lg sm:text-xl md:text-2xl lg:text-[2.125rem] font-bold mb-1 sm:mb-2"
		...
		<div class="p-3 sm:p-4 md:p-6">
			{#if !showRegisterForm}
				...
				<form on:submit={onLoginSubmit} class="space-y-3 sm:space-y-4 md:space-y-6">
					...
					<div>
						<label
							class="block text-xs sm:text-sm md:text-[0.875rem] font-medium mb-1 sm:mb-2"
							style="color: #1d1b21;"
							for="file"
						>
                      ...
                      hover:file:opacity-90 cursor-pointer"
						...
						{#if errors.file}
							<p class="mt-1 sm:mt-2 text-xs sm:text-[0.8125rem]" style="color: #e53e3e;">
								{errors.file}
							</p>
						{/if}
					</div>
					...
					{#if submitStatus === 'success'}
						<div
							class="mt-3 sm:mt-4 p-2 sm:p-3 rounded-[0.35714285714285715rem] text-xs sm:text-[0.8125rem]"
					...
					{:else if submitStatus === 'error'}
						<div
							class="mt-3 sm:mt-4 p-2 sm:p-3 rounded-[0.35714285714285715rem] text-xs sm:text-[0.8125rem]"
							style="background-color: #e53e3e96; color: #1d1b21;"
						>
				...
				<form on:submit={onRegisterSubmit} class="space-y-3 sm:space-y-4 md:space-y-6">
					...
					{:else if registerStatus === 'error'}
						<div
							...
							style="background-color: #e53e3e96; color: #1d1b21;"
						>
							{translations[language].registerError}
							{registerMessage}
						</div>
					{/if}
				</form>
