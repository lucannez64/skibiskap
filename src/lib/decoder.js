...
export function encodeEP(ep) {
    ...
    view.setBigUint64(offset, BigInt(ciphertextLen), true); offset += 8;
    for (let i = 0; i < ciphertextLen; ++i) {
        view.setUint8(offset++, ep.ciphertext[i]);
    }

    view.setBigUint64(offset, BigInt(nonceLen), true); offset += 8;
    ...
    if (nonce2Present) {
        view.setBigUint64(offset, BigInt(nonce2Len), true); offset += 8;
...
/**
 * Encodes ClientEx struct to Uint8Array
 * @param {ClientEx} clientEx
 * @returns {Uint8Array}
 */
function encodeClientEx(clientEx) {
    let clientBytes = encodeClient(clientEx.c);
    let ckBytes = encodeCK(clientEx.id);

    let totalLength = clientBytes.length + ckBytes.length;
    const buffer = new ArrayBuffer(totalLength);
    const view = new Uint8Array(buffer);
    let offset = 0;

    view.set(clientBytes, offset); offset += clientBytes.length;
    view.set(ckBytes, offset); offset += ckBytes.length;
...
export function encodePassword(passwordObject) {
  ...
  function encodeString(str) {
    ...
    view.setBigUint64(offset, bigNum, true); offset += 8;
...
function encodeClient(client) {
    ...
    view.set(kyPublicKeyBytes, offset); offset += kyPublicKeyBytes.length;
    view.set(kySecretKeyBytes, offset); offset += kySecretKeyBytes.length;
    view.set(diPublicKeyBytes, offset); offset += diPublicKeyBytes.length;
    view.set(diSecretKeyBytes, offset); offset += diSecretKeyBytes.length;
    view.setUint8(offset++, secretPresentFlag);
    if (secretPresentFlag) {
        view.set(secretBytes, offset); offset += secretBytes.length;
    }
...
/**
 * Encodes CK struct to Uint8Array
 * @param {CK} ck
 * @returns {Uint8Array}
 */
function encodeCK(ck) {
    let emailBytes = encodeString(ck.email);
    let uuidBytes = ck.id ? ck.id.bytes : new Uint8Array(0);
    let uuidPresentFlag = ck.id ? 1 : 0;
    let kyPublicKeyBytes = encodeKyPublicKey(ck.ky_p);
    let diPublicKeyBytes = encodeDiPublicKey(ck.di_p);

    let totalLength = 0;
    totalLength += emailBytes.length;
    totalLength += 1; // uuid present flag
    if (uuidPresentFlag) {
        totalLength += uuidBytes.length; // uuid bytes (always 16 if present)
    }
    totalLength += kyPublicKeyBytes.length;
    totalLength += diPublicKeyBytes.length;
    ...
    view.set(emailBytes, offset); offset += emailBytes.length;
    view.setUint8(offset++, uuidPresentFlag);
    if (uuidPresentFlag) {
        view.set(uuidBytes, offset); offset += uuidBytes.length;
    }
    view.set(kyPublicKeyBytes, offset); offset += kyPublicKeyBytes.length;
    view.set(diPublicKeyBytes, offset); offset += diPublicKeyBytes.length;
...
function encodeString(str) {
    ...
    view.setBigUint64(offset, BigInt(len), true); offset += 8;
    for (let i = 0; i < len; ++i) {
        view.setUint8(offset++, encodedStr[i]);
    }
    return new Uint8Array(buffer);
...
export function decodePassword(bytes) {
    ...
    function readString(bytes, offset) {
      ...
      const len = Number(view.getBigUint64(offset, true)); offset += 8;
...
export function decodeEP(bytes) {
    ...
    const ciphertextLen = Number(view.getBigUint64(offset, true)); offset += 8;
    if (offset + ciphertextLen > bytes.byteLength) return null; // MalformedData or EndOfStream
    const ciphertext = bytes.slice(offset, offset + ciphertextLen); offset += ciphertextLen;

    const nonceLen = Number(view.getBigUint64(offset, true)); offset += 8;
    if (offset + nonceLen > bytes.byteLength) return null; // MalformedData or EndOfStream
    const nonce = bytes.slice(offset, offset + nonceLen); offset += nonceLen;

    const nonce2PresentFlag = view.getUint8(offset++);
    let nonce2 = null;
    if (nonce2PresentFlag === 1) {
        const nonce2Len = Number(view.getBigUint64(offset, true)); offset += 8;
        if (offset + nonce2Len > bytes.byteLength) return null; // MalformedData or EndOfStream
        nonce2 = bytes.slice(offset, offset + nonce2Len); offset += nonce2Len;
...
function decodeClient(bytes) {
    ...
    const client = { ky_p: ky_p, ky_q: ky_q, di_p: di_p, di_q: di_q, secret: secret };
    return { client: client, remainingBytes: bytes.slice(view.byteOffset + offset) };
}
...
export function uuidToStr(uuid) {
    ...
    const uuids = uuidstring.replace(
      /(.{8})(.{5})(.{5})(.{5})(.{12})/g,
      "$1-$2-$3-$4-$5",
    );
    return uuids;
